(function ($) {
  $(document).ready(function () {
    ...
    $("#graphic-design-skillbar")
      ...
      .on("circle-animation-progress", function (event, progress) {
        $(this)
          .find("strong")
          .html(Math.round(49 * progress) + "<i>%</i>");
      });

    $("#digital-marketing-skillbar")
      .circleProgress({
        value: 0.84,
        size: 118,
        thickness: 2,
  ...
  //     },
  //   });
  const infoList = $(".info-body .title");
  const info = $(".info-body .update-info");
  const image = $(".update-img img");
  if (infoList) {
    for (let i = 0; i < infoList.length; i++) {
      infoList[i].addEventListener("click", function () {
        for (let j = 0; j < infoList.length; j++) {
          if (j == i) {
            image.attr("src", `assets-new/img/home/update-${j + 1}.png`);
            const height =
              info[j].querySelector(".update-info-body").offsetHeight;
            info[j].style.height = height + "px";
          } else {
            info[j].style.height = "0px";
          }
        }
      });
    }
  }
  const footerMenu = $(".footer-dropdown .dropdown-menu");
  const footerLanguage = $(".footer-dropdown .dropdown-toggle");
  if (footerLanguage?.[0]?.offsetWidth && footerMenu?.[0]) {
    footerMenu[0].style.minWidth = footerLanguage?.[0]?.offsetWidth + "px";
  }
